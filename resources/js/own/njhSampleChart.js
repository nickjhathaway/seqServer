...
function njhSampleChart(masterDivId, masterData, downLoadStubName, fracName){
...
function njhSampleChartMips(masterDivId, masterData, downLoadStubName, fracName){
	//Create the margins and set the width and height for the sample chart
	...
	for(pos in this.masterData["tab"]){
		...
		samplePopNames[this.masterData["tab"][actualPos].s_Sample].push({
			...
			h_popUID: this.masterData["tab"][actualPos].h_popUID,
			c_clusterID: this.masterData["tab"][actualPos].c_clusterID,
			c_barcodeFrac: this.masterData["tab"][actualPos].c_barcodeFrac,
			color: this.color(this.masterData["tab"][actualPos].h_popUID) });
	}
	//set up the x domain for the current sample names 
	this.x.domain(this.masterData["tab"].map(function(d) { return d.s_Sample; }));
	...
	bars.enter().append("rect")
	      ...
	      .attr("y", function(d) { 
	      	//adjust height for previously added samples 
	      		var ret = self.y(sampleSum[d.s_Sample] + d.c_barcodeFrac);
	      		sampleSum[d.s_Sample] = sampleSum[d.s_Sample] + d.c_barcodeFrac;
	      		return ret; 
	      		})
	      .attr("height", function(d) { return self.height - self.y(d.c_barcodeFrac); })
	      .attr("width", self.x.rangeBand())
	      .attr("fill", function(d){ return self.color(d.h_popUID);})
	      .style("stroke", "#000")
	      .attr("data-legend",function(d) { return d.h_popUID;})
	      .on("mouseover", function(d){
	      	updateTableWithColors(self.hoverTab,samplePopNames[d.s_Sample], ["s_Sample", "h_popUID","c_clusterID", "c_barcodeFrac"]);
	      	 return self.tooltip.style("visibility", "visible");})
		  .on("mousemove", function(){return self.tooltip.style("top", (d3.event.layerY-10)+"px").style("left",(d3.event.layerX+10)+"px");})
		  .on("mouseout", function(){return self.tooltip.style("visibility", "hidden");});
	bars.exit()
...
njhSampleChartMips.prototype.updateWithData = function(updatedDataTab){
	this.masterData = updatedDataTab;
	this.x.domain(this.masterData["tab"].map(function(d) { return d.s_Sample; }));
	...
	bars.attr("x", function(d) { return self.x(d.s_Sample); })
	      ...
	      .style("stroke", "#000")
	      .attr("data-legend",function(d) { return d.h_popUID;})
	      .on("mouseover", function(d){updateTableWithColors(self.hoverTab,samplePopNames[d.s_Sample], ["s_Sample", "h_popUID","c_clusterID", "c_barcodeFrac"]);
	       return self.tooltip.style("visibility", "visible");})
		  .on("mousemove", function(){return self.tooltip.style("top", (d3.event.layerY-10)+"px").style("left",(d3.event.layerX+10)+"px");})
		  .on("mouseout", function(){return self.tooltip.style("visibility", "hidden");});
    //remove the bars no longer needed
...
function njhSampleChartAllMips(masterDivId, masterData, downLoadStubName, fracName){
	//Create the margins and set the width and height for the sample chart
	...
	//set up the colors and create the info needed for hover tooltip
	this.color = d3.scale.ordinal()
	    .domain(this.masterData["tab"].map(function(d) { return d.c_clusterID; }))
	    .range(this.masterData["popColors"]);
	//initiate the counts to zero and no samples have been added
	var sampleSum = {};
	var samplePopNames = {};
	for(pos in this.masterData["tab"]){
		sampleSum[this.masterData["tab"][pos].mipName] = 0;
		samplePopNames[this.masterData["tab"][pos].mipName] = [];
	}
	for(pos in this.masterData["tab"]){
		var actualPos = this.masterData["tab"].length - 1 - pos;
		samplePopNames[this.masterData["tab"][actualPos].mipName].push({
			mipName: this.masterData["tab"][actualPos].mipName,
			h_popUID: this.masterData["tab"][actualPos].h_popUID,
			c_clusterID: this.masterData["tab"][actualPos].c_clusterID,
			c_barcodeFrac: this.masterData["tab"][actualPos].c_barcodeFrac,
			color: this.color(this.masterData["tab"][actualPos].c_clusterID) });
	}
	//set up the x domain for the current sample names 
	this.x.domain(this.masterData["tab"].map(function(d) { return d.mipName; }));
	this.y.domain([0,1]);
	//add the axes to the chart
	this.chart.append("g")
	      .attr("class", "x axis")
	...
	bars.enter().append("rect")
	      .attr("class", "bar")
	      .attr("x", function(d) { return self.x(d.mipName); })
	      .attr("y", function(d) { 
	      	//adjust height for previously added samples 
	      		var ret = self.y(sampleSum[d.mipName] + d.c_barcodeFrac);
	      		sampleSum[d.mipName] = sampleSum[d.mipName] + d.c_barcodeFrac;
	      		return ret; 
	      		})
	      .attr("height", function(d) { return self.height - self.y(d.c_barcodeFrac); })
	      .attr("width", self.x.rangeBand())
	      .attr("fill", function(d){ return self.color(d.c_clusterID);})
	      .style("stroke", "#000")
	      .attr("data-legend",function(d) { return d.c_clusterID;})
	      .on("mouseover", function(d){
	      	updateTableWithColors(self.hoverTab,samplePopNames[d.mipName], ["mipName", "h_popUID","c_clusterID", "c_barcodeFrac"]);
	      	 return self.tooltip.style("visibility", "visible");})
		  .on("mousemove", function(){return self.tooltip.style("top", (d3.event.layerY-10)+"px").style("left",(d3.event.layerX+10)+"px");})
		  .on("mouseout", function(){return self.tooltip.style("visibility", "hidden");});
	bars.exit()
...
njhSampleChartAllMips.prototype.updateWithData = function(updatedDataTab){
	this.masterData = updatedDataTab;
	this.x.domain(this.masterData["tab"].map(function(d) { return d.mipName; }));
	...
	bars.attr("x", function(d) { return self.x(d.mipName); })
	      .attr("y", function(d) { 
	      		var ret = self.y(sampleSum[d.mipName] + d.c_barcodeFrac);
	      		sampleSum[d.mipName] = sampleSum[d.mipName] + d.c_barcodeFrac;
	      		return ret; 
	      ...
	      .style("stroke", "#000")
	      .attr("data-legend",function(d) { return d.c_clusterID;})
	      .on("mouseover", function(d){updateTableWithColors(self.hoverTab,samplePopNames[d.mipName], ["mipName", "h_popUID","c_clusterID", "c_barcodeFrac"]);
	       return self.tooltip.style("visibility", "visible");})
		  .on("mousemove", function(){return self.tooltip.style("top", (d3.event.layerY-10)+"px").style("left",(d3.event.layerX+10)+"px");})
		  .on("mouseout", function(){return self.tooltip.style("visibility", "hidden");});
    //remove the bars no longer needed
